...
type OAuthAccessTokenRepository interface {
	...
	FindByID(id primitive.Binary) (models.OAuthAccessToken, error)
	Update(id primitive.Binary, user *models.UpdateOAuthAccessToken) error
}
...
func (r *oAuthAccessTokenRepository) FindByID(id primitive.Binary) (models.OAuthAccessToken, error) {
	token := models.OAuthAccessToken{}
	err := r.collection.FindOne(context.TODO(), bson.M{"_id": id, "deleted": false}).Decode(&token)
	return token, err
}

func (r *oAuthAccessTokenRepository) Update(id primitive.Binary, token *models.UpdateOAuthAccessToken) error {
